...
export default async function Footer({ data, siteRepresentation }) {
  ...
  const { isOrganization, siteName, telephone, email, schedulingLink, socialChannels, businessHours, addressLocality, areaServed } = siteRepresentation;
          ...
          <div className="gap-8 grid grid-cols-1 md:grid-cols-5 mb-8">
            ...
            <div className="col-span-1 md:col-span-2">
              ...
              <ul className="space-y-4">
                <li>
                  <NoSSRWrapper>
                    <Link className="flex md:inline-flex items-center justify-center gap-1.5 group text-base text-white/75" href={`mailto:${decodedEmail}`}>
                      <EnvelopeIcon className="h-[1.2em] w-[1.2em] shrink-0" />
                      <span className="group-hover:underline truncate">{decodedEmail}</span>
                    </Link>
                  </NoSSRWrapper>
                </li>
                {telephone &&
                  <li>
                    <NoSSRWrapper>
                      <Link className="flex md:inline-flex items-center justify-center gap-1.5 group text-base text-white/75" href={`tel:${decodedTelephone.replace(/[^\d+]/g, '')}`}>
                        <PhoneIcon className="h-[1.2em] w-[1.2em] shrink-0" />
                        <span className="group-hover:underline truncate">{decodedTelephone}</span>
                      </Link>
                    </NoSSRWrapper>
                  </li>
                }
